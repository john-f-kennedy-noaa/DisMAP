    ...
    try:
        ...
        for version in versions:
            ...
            _archive_folder = os.path.join(
                archive_folder, f"DisMAP_{dismap_tools.date_code(version)}"
            )
            # archive_gdb = rf"{_archive_folder}\DisMAP_{dismap_tools.date_code(version)}.gpkg"

            archive_folders = [
                "initial",
                "results/vector-tabular/metadata",
                "results/raster",
            ]
            ...
            for archiveFolder in archive_folders:
                archiveFolder_path = os.path.abspath(
                    os.path.join(
                        archive_folder,
...
def main(home_folder="", versions="", archive_folder=""):
    try:
        ...
        dis_map_archive_folders(
            home_folder=home_folder, versions=versions, archive_folder=archive_folder
        )
        ...
        arcpy.AddMessage(
            f"Home Folder: {os.path.basename(home_folder)} in '{inspect.stack()[0][3]}'"
        )
        arcpy.AddMessage(
        ...
        for version in versions:
            project_gdb = rf"{home_folder}\{version}\{version}.gdb"
            _archive_folder = os.path.abspath(
                os.path.join(
                    archive_folder,
                    f"DisMAP_{dismap_tools.date_code(version)}",
                    "results/vector-tabular",
                )
            )
            archive_gdb = os.path.abspath(
                rf"{_archive_folder}\DisMAP_{dismap_tools.date_code(version)}.gpkg"
            )
            ...
            arcpy.management.CreateSQLiteDatabase(
                out_database_name=archive_gdb, spatial_type="GEOPACKAGE"
            )
            ...
            for fc in sorted(
                [
                    ...
                    if any(fc.endswith(f"{f}") for f in archive_fcs)
                ]
            ):
                ...
                fc_md.exportMetadata(
                    os.path.abspath(
                        os.path.join(
                            archive_folder,
                            f"DisMAP_{dismap_tools.date_code(version)}",
                            f"results/vector-tabular/metadata/{fc}.xml",
                        )
                    ),
                    "ISO19139",
                    "REMOVE_ALL_SENSITIVE_INFO",
