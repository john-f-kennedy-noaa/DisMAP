...
def raster_properties_report(dataset=""):
    try:
        if not dataset:
            ...
            arcpy.AddMessage(
                f"\t\t\t Spatial Reference: {raster.spatialReference.name}"
            )
            arcpy.AddMessage(
                f"\t\t\t XYResolution:      {raster.spatialReference.XYResolution} {raster.spatialReference.linearUnitName}s"
            )
            arcpy.AddMessage(
                f"\t\t\t XYTolerance:       {raster.spatialReference.XYTolerance}  {raster.spatialReference.linearUnitName}s"
            )
            arcpy.AddMessage(
                f"\t\t\t Extent:            {raster.extent.XMin} {raster.extent.YMin} {raster.extent.XMax} {raster.extent.YMax} (XMin, YMin, XMax, YMax)"
            )
            arcpy.AddMessage(
                f"\t\t\t Cell Size:         {raster.meanCellHeight}, {raster.meanCellWidth} (H, W)"
            )
            arcpy.AddMessage(
                f"\t\t\t Rows, Columns:     {raster.height} {raster.width} (H, W)"
            )
            arcpy.AddMessage(
                f"\t\t\t Statistics:        {raster.minimum} {raster.maximum} {raster.mean} {raster.standardDeviation} (Min, Max, Mean, STD)"
            )
            arcpy.AddMessage(
                f"\t\t\t Pixel Type:        {pixel_types[raster.pixelType]}"
            )
...
def create_alasaka_bathymetry(project_folder=""):
    try:
        ...
        ai_bathy_raster = (
            rf"{project_folder}\Bathymetry\Bathymetry.gdb\AI_IDW_Bathy_Raster"
        )
        ebs_bathy_raster = (
            rf"{project_folder}\Bathymetry\Bathymetry.gdb\EBS_IDW_Bathy_Raster"
        )
        goa_bathy_raster = (
            rf"{project_folder}\Bathymetry\Bathymetry.gdb\GOA_IDW_Bathy_Raster"
        )

        ai_bathymetry = rf"{project_folder}\Bathymetry\Bathymetry.gdb\AI_IDW_Bathymetry"
        ebs_bathymetry = (
            rf"{project_folder}\Bathymetry\Bathymetry.gdb\EBS_IDW_Bathymetry"
        )
        goa_bathymetry = (
            rf"{project_folder}\Bathymetry\Bathymetry.gdb\GOA_IDW_Bathymetry"
        )
        enbs_bathymetry = (
            rf"{project_folder}\Bathymetry\Bathymetry.gdb\ENBS_IDW_Bathymetry"
        )
        nbs_bathymetry = (
            rf"{project_folder}\Bathymetry\Bathymetry.gdb\NBS_IDW_Bathymetry"
        )
        ...
        arcpy.AddMessage(
            f"Spatial Reference for {os.path.basename(ai_bathy)}: {spatial_ref}"
        )
        ...
        arcpy.AddMessage(
            "Converting AI_IDW_Bathy_Grid from positive values to negative"
        )
        ...
        arcpy.AddMessage(
            "Converting EBS_IDW_Bathy_Grid from positive values to negative"
        )
        ...
        arcpy.AddMessage(
            "Setting values equal to and less than 0 in the GOA_IDW_Bathy_Grid Null values"
        )
        ...
        arcpy.AddMessage(
            "Converting the GOA_IDW_Bathy_Grid from positive values to negative"
        )
        ...
        arcpy.AddMessage(
            "Appending the AI raster to the GOA grid to ensure complete coverage"
        )

        extent = arcpy.Describe(goa_bathy_raster).extent
        X_Min, Y_Min, X_Max, Y_Max = (
            extent.XMin - (1000 * 366),
            extent.YMin - (1000 * 80),
            extent.XMax,
            extent.YMax,
        )
        ...
        arcpy.management.Append(
            inputs=ai_bathy_raster,
            target=goa_bathy_raster,
            schema_type="TEST",
            field_mapping="",
            subtype="",
        )
        ...
        arcpy.AddMessage(
            "Appending the EBS raster to the AI grid to ensure complete coverage"
        )
        ...
        arcpy.management.Append(
            inputs=ebs_bathy_raster,
            target=ai_bathy_raster,
            schema_type="TEST",
            field_mapping="",
            subtype="",
        )
...
def create_hawaii_bathymetry(project_folder=""):
    try:
        ...
        arcpy.conversion.PolygonToRaster(
            in_features=hi_bathy_grid,
            value_field="Depth_MEDI",
            out_rasterdataset=hi_bathy_raster,
...
def gebco_bathymetry(project_folder=""):
    try:
        ...
        for table_name in gebco_dict:
            ...
            bathy_raster = (
                rf"{project_folder}\Bathymetry\Bathymetry.gdb\{table_name}_Bathy_Raster"
            )
            bathymetry = (
                rf"{project_folder}\Bathymetry\Bathymetry.gdb\{table_name}_Bathymetry"
            )

            arcpy.AddMessage(
                f"Copy GEBCO File: {os.path.basename(gebco_grid)} to {os.path.basename(bathy_grid)}"
            )
            ...
            arcpy.management.DefineProjection(
                bathy_grid, gebco_grid.replace(".asc", ".prj")
            )
            arcpy.AddMessage(
                "\tDefine Projection: {0}\n".format(
                    arcpy.GetMessages().replace("\n", "\n\t")
            ...
            arcpy.AddMessage(
                f"Project Raster to create: {os.path.basename(bathy_raster)}"
            )
            ...
            arcpy.management.ProjectRaster(
                in_raster=bathy_grid, out_raster=bathy_raster, out_coor_system=region_sr
            )
            arcpy.AddMessage(
                "\tProject Raster: {0}".format(
                    arcpy.GetMessages().replace("\n", "\n\t")
                )
            )
            ...
            arcpy.AddMessage(
                f"Set Null for positive elevation values to create: {os.path.basename(bathymetry)}"
            )
            with arcpy.EnvManager(
                scratchWorkspace=arcpy.env.scratchGDB, workspace=arcpy.env.workspace
            ):
    ...
    try:
        ...
        if not arcpy.Exists(rf"{project_folder}\Bathymetry\Bathymetry.gdb"):
            ...
            arcpy.management.CreateFileGDB(
                rf"{project_folder}\Bathymetry", "Bathymetry"
            )
            get_messages = "\t" + arcpy.GetMessages().replace("\n", "\n\t") + "\n"
            arcpy.AddMessage(f"{get_messages}")
            del get_messages
