...
def director(project_gdb="", Sequential=True, table_names=[]):
    try:
        ...
        for dataset in datasets:
            ...
            arcpy.AddMessage(
                f"\tCopying the {dataset_name} Table to the project GDB Table"
            )
            arcpy.management.Copy(
                rf"{region_gdb}\{dataset_name}", rf"{project_gdb}\{dataset_name}"
            )
            arcpy.AddMessage(
                "\tCopy: {0} {1}\n".format(
                    dataset_name, arcpy.GetMessages(0).replace("\n", "\n\t")
                )
            )
            ...
            fields = [
                f.name
                for f in arcpy.ListFields(rf"{project_gdb}\{dataset_name}")
                if f.type == "String"
            ]
            # Create update cursor for feature class
            with arcpy.da.UpdateCursor(
                rf"{project_gdb}\{dataset_name}", fields
            ) as cursor:
                for row in cursor:
        ...
        try:
            ...
            if Test:
                director(
                    ...
                    Sequential=True,
                    table_names=["GMEX_IDW", "HI_IDW", "WC_ANN_IDW", "WC_TRI_IDW"],
                )
