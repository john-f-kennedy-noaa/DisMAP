...
def worker(region_gdb=""):
    ...
    # Start: Create new LayerSpeciesYearImageName table
        ...
        arcpy.AddMessage(f"\t\tAlter Fields for: '{os.path.basename(layer_species_year_image_name)}'")
        ...
        arcpy.AddMessage(f"\t\tImport Metadata for: '{os.path.basename(layer_species_year_image_name)}'")
        dismap_tools.import_metadata(project_folder, layer_species_year_image_name)
