...
def printRowContent(region_indicators):
    try:
        ...
        fields = [
            f.name
            for f in arcpy.ListFields(region_indicators)
...
def worker(region_gdb=""):
    try:
        ...
        if not arcpy.Exists(rf"{region_gdb}"):
            ...
            arcpy.AddError(
                f"Function: '{inspect.stack()[0][3]}', Line Number: {inspect.stack()[0][2]}"
            )
            sys.exit()
        else:
            pass
        ...
        arcpy.AddMessage(
            f"\tGet list of vaules for the {table_name} Indicators table from the Datasets table"
        )
        ...
        with arcpy.da.SearchCursor(
            layerspeciesyearimagename,
            fields,
            where_clause=f"DatasetCode = '{datasetcode}'",
        ) as cursor:
            for row in cursor:
                image_name, variable, species, commonname, corespecies, year = (
                    row[0],
                    row[1],
                    row[2],
                    row[3],
                ...
                if "Species Richness" not in variable:
                    ...
                    input_raster_path = (
                        rf"{image_folder}\{table_name}\{variable}\{image_name}.tif"
                    )
        ...
        for variable in sorted(input_rasters):
            ...
            for raster_year in sorted(raster_years):
                ...
                if PrintRecord:
                    ...
                    arcpy.AddMessage(
                        f"\t\t> Output Raster: {os.path.basename(input_raster_path)}"
                    )
                del PrintRecord

                arcpy.AddMessage(
                    f"Processing {image_name} Biomass Raster for year: {raster_year}"
                )

                # Get maximumBiomass value to filter out "zero" rasters
                maximumBiomass = float(
                    arcpy.management.GetRasterProperties(
                        input_raster_path, "MAXIMUM"
                    ).getOutput(0)
                )
                ...
                if maximumBiomass > 0.0:
                    ...
                    biomassArray = arcpy.RasterToNumPyArray(
                        input_raster_path, nodata_to_value=np.nan
                    )
                    ...
                    arcpy.AddMessage(
                        f"\t> biomassArray non-nan count: {np.count_nonzero(~np.isnan(biomassArray))}"
                    )
                    ...
                    # Latitude
                    latitudeArray = arcpy.RasterToNumPyArray(
                        region_latitude, nodata_to_value=np.nan
                    )
                    ...
                    sortedBiomassArrayQuantile = sortedBiomassArrayCumSum / np.nansum(
                        flatBiomassArray
                    )
                    ...
                    OffsetLatitude = (
                        CenterOfGravityLatitude - first_year_offset_latitude
                    )

                    weightedLatitudeArrayVariance = np.nanvar(weightedLatitudeArray)
                    weightedLatitudeArrayCount = np.count_nonzero(
                        ~np.isnan(weightedLatitudeArray)
                    )

                    CenterOfGravityLatitudeSE = math.sqrt(
                        weightedLatitudeArrayVariance
                    ) / math.sqrt(weightedLatitudeArrayCount)
                    ...
                    arcpy.AddMessage(
                        f"\t\t> Center of Gravity Latitude: {CenterOfGravityLatitude}"
                    )
                    arcpy.AddMessage(
                        f"\t\t> Minimum Latitude (5th Percentile): {MinimumLatitude}"
                    )
                    arcpy.AddMessage(
                        f"\t\t> Maximum Latitude (95th Percentile): {MaximumLatitude}"
                    )
                    arcpy.AddMessage(f"\t\t> Offset Latitude: {OffsetLatitude}")
                    arcpy.AddMessage(
                        f"\t\t> Center of Gravity Latitude Standard Error: {CenterOfGravityLatitudeSE}"
                    )
                    ...
                    longitudeArray = arcpy.RasterToNumPyArray(
                        region_longitude, nodata_to_value=np.nan
                    )
                    ...
                    del (
                        sortedBiomassArrayCumSum,
                        sortedBiomassArrayQuantile,
                        diffArray,
                        minIndex,
                    )
                    ...
                    CenterOfGravityLongitude = (
                        sumWeightedLongitudeArray / sumBiomassArray
                    )
                    ...
                    OffsetLongitude = (
                        CenterOfGravityLongitude - first_year_offset_longitude
                    )

                    weightedLongitudeArrayVariance = np.nanvar(weightedLongitudeArray)
                    weightedLongitudeArrayCount = np.count_nonzero(
                        ~np.isnan(weightedLongitudeArray)
                    )

                    CenterOfGravityLongitudeSE = math.sqrt(
                        weightedLongitudeArrayVariance
                    ) / math.sqrt(weightedLongitudeArrayCount)
                    ...
                    # Added/Modified by JFK June 15, 2022
                    CenterOfGravityLongitude = (
                        np.mod(CenterOfGravityLongitude - 180.0, 360.0) - 180.0
                    )
                    ...
                    arcpy.AddMessage(
                        f"\t\t> Center of Gravity Longitude: {CenterOfGravityLongitude}"
                    )

                    # arcpy.AddMessage(F"\t\t> Center of Gravity Longitude: {np.mod(CenterOfGravityLongitude - 180.0, 360.0) -180.0}")

                    arcpy.AddMessage(
                        f"\t\t> Minimum Longitude (5th Percentile): {MinimumLongitude}"
                    )
                    arcpy.AddMessage(
                        f"\t\t> Maximum Longitude (95th Percentile): {MaximumLongitude}"
                    )
                    arcpy.AddMessage(f"\t\t> Offset Longitude: {OffsetLongitude}")
                    arcpy.AddMessage(
                        f"\t\t> Center of Gravity Longitude Standard Error: {CenterOfGravityLongitudeSE}"
                    )

                    del (
                        longitudeArray,
                        weightedLongitudeArray,
                        sumWeightedLongitudeArray,
                    )
                    ...
                    # Bathymetry
                    bathymetryArray = arcpy.RasterToNumPyArray(
                        region_bathymetry, nodata_to_value=np.nan
                    )
                    ...
                    arcpy.AddMessage(
                        f"\t\t> Sum Weighted Bathymetry: {sumWeightedBathymetryArray}"
                    )
                    ...
                    weightedBathymetryArrayCount = np.count_nonzero(
                        ~np.isnan(weightedBathymetryArray)
                    )

                    CenterOfGravityDepthSE = math.sqrt(
                        weightedBathymetryArrayVariance
                    ) / math.sqrt(weightedBathymetryArrayCount)

                    del weightedBathymetryArrayVariance, weightedBathymetryArrayCount

                    arcpy.AddMessage(
                        "\t\t> Center of Gravity Depth: {0}".format(
                            CenterOfGravityDepth
                        )
                    )

                    arcpy.AddMessage(
                        "\t\t> Minimum Depth (5th Percentile): {0}".format(MinimumDepth)
                    )

                    arcpy.AddMessage(
                        "\t\t> Maximum Depth (95th Percentile): {0}".format(
                            MaximumDepth
                        )
                    )
                    ...
                    arcpy.AddMessage(
                        "\t\t> Center of Gravity Depth Standard Error: {0}".format(
                            CenterOfGravityDepthSE
                        )
                    )
