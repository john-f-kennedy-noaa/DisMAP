...
def worker(project_gdb="", csv_file=""):
    try:
        ...
        field_csv_dtypes = dev_dismap_tools.dTypesCSV(csv_data_folder, table_name)
        field_gdb_dtypes = dev_dismap_tools.dTypesGDB(csv_data_folder, table_name)
        ...
        #encoding, index_column = dev_dismap_tools.get_encoding_index_col(csv_file)
        ...
        # Alter Fields
        dev_dismap_tools.alter_fields(csv_data_folder, dataset_path)
        # Load Metadata
        ...
        # Imports
        del dev_dismap_tools, md, pd, np, warnings
        # Function parameters
...
def update_datecode(csv_file="", project_name=""):
    try:
        ...
        arcpy.AddMessage(f"\tNew Date Code: {dev_dismap_tools.date_code(project_name)}")
        df = df.replace(regex = old_date_code, value = dev_dismap_tools.date_code(project_name))
...
if __name__ == "__main__":
    try:
        ...
        #sys.path.append(os.path.dirname(os.path.dirname(project_gdb)))
