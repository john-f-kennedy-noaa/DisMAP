...
data <- rbind(ai, ebs, gmex, goa, nbs, neus_fall, neus_spri, seus_fall, seus_spri, seus_sum, wc_ann, wc_tri)

myfilt <- data %>% 
  filter(DistributionProjectName == "NMFS/Rutgers IDW Interpolation") %>% 
  select(region, year, spp, haulid, wtcpue) %>% 
  filter(wtcpue > 0) %>%
  distinct(region, year, spp) %>% 
  group_by(region, spp) %>% 
  summarize(n = n())

myfilt$max <- ifelse(myfilt$region == "Aleutian Islands", 13,
                     ifelse(myfilt$region == "Eastern Bering Sea", 36,
                            ifelse(myfilt$region == "Gulf of Alaska", 15,
                                   ifelse(myfilt$region == "Gulf of Mexico", 12,
                                          ifelse(myfilt$region == "Northeast US Fall", 48,
                                                 ifelse(myfilt$region == "Northeast US Spring", 46,
                                                        ifelse(myfilt$region == "Northern Bering Sea", 6,
                                                               ifelse(myfilt$region == "Southeast US Fall", 33,
                                                                      ifelse(myfilt$region == "Southeast US Spring", 32,
                                                                             ifelse(myfilt$region == "Southeast US Summer", 32,
                                                                                    ifelse(myfilt$region == "West Coast Annual", 20,
                                                                                           ifelse(myfilt$region == "West Coast Triennial", 10, NA))))))))))))
