...
```{r  haul freq inc vs dec, eval= FALSE}
...
```{r}
# This code is pretty generic and the cbind() approach can be applied to
# GAMs, brms, GLMs, sdmTMB, glmmTMB, etc
set.seed(123)
d <- data.frame(year = 1:20,
                n_tows = ceiling(100 * runif(20, 7, 10)),
                present_tows = ceiling(100 * runif(20, 1, 3)))

# example of linear trend using the cbind() approach, which we'll call the compact model
fit <- glm(
  cbind(present_tows, n_tows - present_tows) ~ year,
  data = d,
  family = binomial
)
summary(fit)

# example of using an indicator testing if last 10 years is different
d$last10 <- ifelse(d$year > 10, 1, 0)
fit2 <- glm(
  cbind(present_tows, n_tows - present_tows) ~ last10,
  data = d,
  family = binomial
)


# You could also replicate the rows to be individual observations
d_long <- do.call(rbind,
                  lapply(1:nrow(d), function(i) {
                    data.frame(
                      year = d$year[i],
                      tow_present = c(rep(1, d$present_tows[i]),
                                      rep(0, d$n_tows[i] - d$present_tows[i]))
                    )
                  })
)
fit_long <- glm(
...
# compare trends -- they're exactly the same
